...
### Fixed
- 2026-06-02 — **Two client-reported OTP-flow bugs** in the sign-in surface:
  - **"After entering pin the button is not responsive"** — the login OTP `Confirm` button (`signin/otp_screen.dart`) used to be gated on `isOtpComplete && !_isVerifying && !_isDisposed`. A focus/setState race in the per-cell `onChanged` handler could leave `isOtpComplete` stale even when the 6 cells visibly held digits, producing a dead button. Fix: the button is now tappable whenever `!_isVerifying && !_isDisposed`. The internal length check in `_verifyOtp` still shows "Please enter complete OTP" if fewer than 6 digits are typed. `isOtpComplete` is retained only for the disabled-look styling. **Bonus fix in the same screen:** `_showMessage` always called `CustomSnackBar.showError` regardless of the `isError` flag — success messages (e.g. "OTP resent successfully") were rendered red. Now respects the flag.
  - **Signup OTP twin of the same bug** — `splash/signup_screen.dart` case 2 on a failed verify called both `provider.setOtp("")` AND `isOtpValid = false`. That cleared the provider state and disabled the AuthLayout Continue button, but did NOT clear the child `StepOtp._controllers`, so the user saw 6 filled cells + a disabled "unresponsive" button. Fix: drop the `provider.setOtp("")` and `isOtpValid = false` reset; only clear `_isProcessing`. The user's input stays intact, the button stays responsive, and they can either re-tap (in case the previous failure was transient) or edit a single digit.
  - **"When OTP fails to send it doesn't issue a resend"** — two problems compounded:
    - `signin/otp_screen.dart::_resendOtp` ignored the `{status, message}` envelope from `provider.sendLoginOtp` and ran the success path unconditionally. A silently-rejected resend now showed "OTP resent successfully" AND reset the 30 s cooldown, locking the user out of Resend for half a minute. Fix: inspect the envelope; only restart the cooldown and clear cells on `status: true`; on failure, show the backend's error message and leave the cooldown alone so the user can immediately retry.
    - `signin/login_screen.dart::_handleContinue` on a failed first send used to show only a transient 3 s snackbar before disappearing — no persistent retry affordance. Fix: alongside the toast, set a new `_lastSendError` field that drives a persistent inline error chip below the phone input (red border on the field + an error-icon chip with the message). The Continue button label flips to "Try again" while the error is shown. The chip clears when the user edits the phone number or a subsequent send succeeds.
  - Files touched: `lib/presentation/screens/signin/otp_screen.dart`, `lib/presentation/screens/signin/login_screen.dart`, `lib/presentation/screens/splash/signup_screen.dart`. Verified: `flutter analyze` on these three files = 0 errors, no new warnings (one pre-existing `_showSuccess unused_element` warning in signup_screen.dart is unchanged). Backend `dispatchAfterResponse` WhApi behaviour (server always returns `status:true` regardless of delivery) is by design — the client-side resend timer + Try-again chip is the user-facing mitigation; a synchronous-send Phase 2 server hardening would be a separate task.
