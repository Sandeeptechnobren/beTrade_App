...
class _ExplorePageState extends State<ExplorePage> {
  ...
  void _onFocusChange() {
    ...
    setState(() {
      ...
      // Only show history when focused AND search box is empty
  ...
  // 🔥 CRITICAL: Search that DOES NOT auto-clear
  void _performSearch(String value) {
    ...
    _debounce = Timer(const Duration(milliseconds: 300), () {
      ...
      try {
        ...
        if (value.trim().isEmpty) {
          // 🔥 IMPORTANT: Do NOTHING when search is empty
  ...
  // 🔥 CRITICAL: Clear search ONLY when cross button is clicked
  void _clearSearch() {
    if (_isDisposed || !mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    // 🔥 Show search results if searching, otherwise show all trades
