...
class _TermsOfServicePageState extends State<TermsOfServicePage> {
  ...
  void _loadCachedTerms() {
    final cached = LocalStorage.getCachedData("terms_conditions");
    if (cached != null) {
      try {
        setState(() {
          _apiData = jsonDecode(cached);
          _isLoading = false;
        });
      } catch (e) {
  ...
  Future<void> _fetchTermsConditions() async {
    try {
      ...
      final fullUrl = '$cleanBaseUrl/termsConditions/list';
      ...
      if (ok && data['data'] != null && data['data'].isNotEmpty) {
        ...
        setState(() {
          _errorMessage = "No terms & conditions found";
          _isLoading = false;
        });
      }
      ...
      setState(() {
        _errorMessage =
            e.response?.data?['message'] ?? "Failed to load terms & conditions";
        _isLoading = false;
      });
    } catch (e) {
      debugPrint("ERROR => $e");
  ...
  /// separator). The backend ships a flat numbered list — the Figma
  /// shows section headings but the source has none, so each block
  /// renders as a single body paragraph at Figma weight/size.
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              ..._parseAndBuildContent(
                                _apiData?['data']?[0]?['terms_and_condition'] ??
                                    "",
                              ),
                            ],
                          ),
  ...
  /// Figma "Frame 2609862" — only a back chip on the left, 1px hairline
  /// at the bottom. The screen title isn't part of this strip; it
