...
5. **Check for security issues**:
   ...
   - Auth tokens stored insecurely (plain SharedPreferences in Flutter; localStorage in web)
...
9. **BLAST RADIUS DAMAGE CHECK**:
   **CRITICAL** (auto-revert unless explicitly requested):
   ...
   - Lock files modified (`package-lock.json`, `bun.lock`, `yarn.lock`, `pubspec.lock`) → only valid if dependencies were intentionally changed

   **WARNING** (flag for human review):
   - Infrastructure files modified (`docker-compose.yml`, `Dockerfile`, `nginx.conf`, `.github/workflows/*`, `Procfile`, `ecosystem.config.js`, `codemagic.yaml`) → deployment-affecting changes need explicit approval
   - CI/CD pipeline files modified → can break all future deployments
   - Shared config files modified (`.env.example`, `analysis_options.yaml`, `pubspec.yaml`, `tsconfig.json`, `next.config.*`, `vite.config.*`) → affects entire project
   - Files modified OUTSIDE the module scope of the current task → check if this was intentional or accidental scope creep
   - Production-only files modified (server configs, cron jobs, systemd units, signing configs) → verify these changes were part of the plan
