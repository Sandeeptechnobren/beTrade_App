...
> **Project note for Flutter**: many of these checks (CORS, SQL injection, rate limiting) belong on the backend (`api.buildacademy.io`) and cannot be enforced from this client repo. Client-side security tests should focus on: token storage location, HTTPS enforcement, error-message safety, and cleartext-traffic config.
