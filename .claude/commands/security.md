...
- Where are auth tokens stored? (httpOnly cookies vs localStorage; for Flutter: `flutter_secure_storage` vs plain `SharedPreferences`)
- Do sessions have expiry? What's the TTL?
- Do password reset / OTP links expire?
...
- Is `.env` in `.gitignore`? **For Flutter: check whether `.env` is also bundled as a Flutter asset (`pubspec.yaml`).**
...
- Run: `npm audit` / `flutter pub outdated --mode=null-safety` for dependency vulnerabilities
- HTTPS enforced? **For Android: check `usesCleartextTraffic` in `AndroidManifest.xml`.**
- Search for `console.log` / `print` / `debugPrint` that output user objects, request bodies, or auth tokens. Do any log statements capture PII (emails, passwords, payment data, IP addresses)?
