...
> **Project note**: This Flutter app has **no application database** — all persistence is HTTP-mediated via the backend at `api.buildacademy.io`. The `/db` command applies only when a backend or DB server is added. Until then, this command will fail at the SSH pre-check above.
