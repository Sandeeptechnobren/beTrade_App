...
## Purpose
Static assets bundled into the Flutter app. Three subfolders: `images/`, `logo/`, and `fonts/` (SFProRounded family, single weight). The app's `.env` file at the repo root is **also** bundled as a Flutter asset (declared in `pubspec.yaml`) — see `docs/CODEBASE_AUDIT.md` §9 for why this is a concern.
...
- `logo/app_icon.png` — source image for `flutter_launcher_icons` (configured in `pubspec.yaml`). Adaptive Android icon background `#ffffff`.
...
## Dependencies
- Outbound: none.
- Inbound: registered under `flutter.assets` in `pubspec.yaml` (`assets/images/`, `assets/logo/`, plus `.env`); referenced from `lib/presentation/*` screens; `pubspec.yaml`'s `flutter_icons.image_path` points at `assets/logo/app_icon.png`.
...
- Keep filenames lowercase + hyphens for PNG/JPG; PascalCase / camelCase image names are inconsistent and harder to reference reliably across platforms.
